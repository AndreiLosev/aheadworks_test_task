...
export const ActionType = {
    ...
    thuncAddBasket: (productId, quantity, productProps, inBasketid) => async () => {
        if (quantity === 0) {
        ...
        if (result) {
            setTimeout(async () => await Api.addBasket(productId, quantity, productProps), 500)
            return;
        }

        await Api.addBasket(productId, quantity, productProps);
    }
}
